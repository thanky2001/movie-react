                    ...
                    return (
                        <ListItem key={index}>
                            ...
                            <div className="showtime">
                                ...
                                { 
                                lst.lstLichChieuTheoPhim.length ?
                                    <div className='time'>
                                        <span className="name--film">Lịch chiếu:</span>
                                        {lst.lstLichChieuTheoPhim.map((st,index)=>{
                                            return(
                                                <Link key={index} to={`/checkout/${st.maLichChieu}-${ToSlug(st.tenRap)}-${ToSlug(lst.tenPhim)}`} className="btn btn--showtime">{splitDateString(st.ngayChieuGioChieu)[1].slice(0,5)}</Link>
                                            )
                                        })}
                                    </div>
                                    : ''
